...
describe('Login Page', () => {
  it('Renderización correcta del componente', async () => {
    ...
    expect(logoUnsam).toHaveAttribute(
      'src',
      expect.stringContaining('logo-unsam-largo.png')
    )
  ...
  it('Mensaje de error por mail incorrecto o sin llenar', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Ingrese una dirección de email válida/i)
      ).toBeInTheDocument()
    })
    ...
    // verifica que se muestra el mensaje de error para el email
    await waitFor(() => {
      expect(screen.getByText(/Debe ingresar un email/i)).toBeInTheDocument()
    })
  })
  ...
  it('Mensaje de error por password demasiado corta o sin llenar', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Debe ingresar una contraseña')
      ).toBeInTheDocument()
    })
  })
